...
export default function AppLayout({ children }: { children: ReactNode }) {
    ...
    const currentLabel = appNav.find(
        (item) => pathname === item.href || pathname?.startsWith(item.href + "/")
    )?.label ?? "App";

    return (
        <div className="flex h-screen overflow-hidden" style={{ "--section-accent": "var(--primary)" } as React.CSSProperties}>
            {/* Sidebar */}
            <aside
